...
impl NodeComponent for InaccessibleBool {
    async fn process(&self, _ctx: &liteflow_rust::CmpContext) -> Result<Value, LiteflowError> {
        panic!("不可访问的节点不应被执行");
    }
    fn is_access(&self, _ctx: &liteflow_rust::CmpContext) -> bool {
        false
    }
}
...
fn script_validate_ex() {
    let exec = liteflow_rust::script::script_executor::RhaiScriptExecutor::new();
