...
/// GitHub Copilot provider，带自动 OAuth 与 token 刷新。
...
impl CopilotProvider {
    /// 创建 Copilot provider。
    ///
    /// - `github_token`：可选的 GitHub access token；为 None 时走设备码流。
    pub fn new(github_token: Option<&str>) -> Self {
        let token_dir = directories::ProjectDirs::from("", "", "liteflow-rust")
            .map(|dir| dir.config_dir().join("copilot"))
            .unwrap_or_else(|| {
                let user = std::env::var("USER")
                    .or_else(|_| std::env::var("USERNAME"))
                    .unwrap_or_else(|_| "unknown".to_string());
                std::env::temp_dir().join(format!("liteflow-rust-copilot-{user}"))
            });
    ...
    async fn send_chat_request(
        ...
        if !response.status().is_success() {
            return Err(api_error("GitHub Copilot", response).await);
        }
