...
//
// 本文件衍生自 ZeroClaw 项目 src/providers/glm.rs。
// 修改：
// - import 路径调整为 liteflow-agent-provider-core；
// - 修正 zeroclaw 原始代码的 bug（原 chat_with_history/warmup 引用不存在的 self.client，
//   现统一为结构体持有的 client 字段，复用连接池）。
...
impl GlmProvider {
    /// 从 `id.secret` 格式的 API key 创建 GLM provider。
...
mod tests {
    ...
    #[test]
    fn parses_api_key() {
        let p = GlmProvider::new("abc123.secretXYZ");
    ...
    fn handles_invalid_key_format() {
        let p = GlmProvider::new("no-dot-here");
    ...
    fn generates_jwt_token() {
        let p = GlmProvider::new("testid.testsecret");
