...
//
// 本文件衍生自 ZeroClaw 项目 src/providers/traits.rs。
// 修改：import 路径调整为 liteflow-agent-provider-core 模块布局；
//       新增 chat_owned 默认方法以支持 'static 生命周期的 Model::stream 桥接。
...
pub struct ChatResponse {
    ...
    pub quota_metadata: Option<crate::quota_types::QuotaMetadata>,
}
...
pub trait Provider: Send + Sync {
    ...
    /// Owned 版本的 chat：接收 `Vec<ChatMessage>` 而非借用。
    ///
    /// 用于桥接 agentscope `Model::stream()`（返回 `'static` Stream）与
    /// zeroclaw `Provider::chat()`（借用 messages）。默认实现转调 `chat()`。
    ///
    /// 这是 LiteFlow-Rust 新增的方法，不在 zeroclaw 原始 trait 中。
    async fn chat_owned(
        &self,
        ...
        let tools_ref: Option<&[ToolSpec]> = tools.as_deref();
