...
//
// 本文件衍生自 ZeroClaw 项目 src/auth/mod.rs。
// 修改：
// - 删除 from_config / state_dir_from_config（解耦对 zeroclaw Config 的依赖），
//   保留 AuthService::new(state_dir, encrypt_secrets) 作为唯一构造入口；
// - 增加 secrets 模块声明（原在 zeroclaw security/）。
...
#[derive(Clone)]
/// 统一管理提供商认证配置、令牌持久化与 OAuth 刷新。
///
/// 对应 ZeroClaw `auth::AuthService`，由 LiteFlow provider-core 独立承载。
...
impl AuthService {
    /// 从状态目录构造 AuthService（encrypt_secrets 控制是否加密存储）。
    ///
    /// 这是解耦 zeroclaw Config 后的唯一构造入口。
