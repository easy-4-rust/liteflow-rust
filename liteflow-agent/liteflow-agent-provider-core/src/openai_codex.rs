...
//
// 本文件衍生自 ZeroClaw 项目 src/providers/openai_codex.rs。
// 修改：import 路径调整为 liteflow-agent-provider-core；
// ProviderRuntimeOptions 改用本 crate 的精简版（去 CompatibleApiMode）。
// "ZeroClaw" 是 ZeroClaw Labs 的商标；本项目与其无官方关联。

use crate::auth::AuthService;
use crate::auth::openai_oauth::extract_account_id_from_jwt;
use crate::multimodal;
use crate::runtime_options::ProviderRuntimeOptions;
...
impl Provider for OpenAiCodexProvider {
    ...
    async fn chat_with_system(
        ...
        let config = crate::multimodal::MultimodalConfig::default();
