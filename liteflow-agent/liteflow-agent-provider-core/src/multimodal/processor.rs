...
//
// 本文件衍生自 ZeroClaw 项目 src/multimodal.rs。
// 修改：
// - 内联 MultimodalConfig（原在 zeroclaw config/schema.rs）；
// - 用 provider-core 的 proxy::build_client_with_timeouts 替代 crate::config::build_runtime_proxy_client_with_timeouts；
...
pub async fn prepare_messages_for_provider(
    ...
    let remote_client = build_client_with_timeouts(30, 10, &ProxyConfig::default());
...
mod tests {
    ...
    #[test]
    fn effective_limits_clamps_values() {
        let config = MultimodalConfig {
            max_images: 100,
            max_image_size_mb: 100,
            allow_remote_fetch: false,
        };
        let (images, size) = config.effective_limits();
        assert_eq!(images, 16);
        assert_eq!(size, 20);
    }
}
