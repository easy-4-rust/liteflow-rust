...
async fn main() {
    ...
    bus.register(
        "riskCheck",
        cmp(|_| async move { Err(LiteflowError::Custom("风控拦截模拟".into())) }),
    );
    bus.register(
        "riskFallback",
        cmp(|_| async move {
    ...
    let resp = bus
        .execute_with_data("orderChain", json!({"orderId": "A001"}))
        .await;
    ...
    bus.reload_chain(
        "orderChain",
        "THEN(checkStock, WHEN(queryPrice, queryCoupon), normalPrice, pay)",
    )
    .unwrap();
