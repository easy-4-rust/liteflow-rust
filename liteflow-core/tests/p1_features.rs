...
use liteflow_core::{cmp, rule, FlowBus, LiteflowError, ScriptKind};
...
async fn script_boolean_in_if() {
    ...
    bus.register("pass", cmp(|ctx| async move {
        ctx.set_data("result", json!("pass"));
        Ok(Value::Null)
    }));
    bus.register("fail", cmp(|ctx| async move {
...
async fn script_switch_and_for() {
    let bus = FlowBus::new();
    bus.register_script_typed("sw", "rhai", ScriptKind::Switch, r#"input.route"#).unwrap();
    bus.register_script_typed("counter", "rhai", ScriptKind::For, "3").unwrap();
    ...
    bus.add_chain("c1", "THEN(SWITCH(sw).TO(a, b), FOR(counter).DO(b))").unwrap();
...
async fn script_type_error_checked() {
    ...
    bus.register_script_typed("bad", "rhai", ScriptKind::Boolean, r#""not a bool""#).unwrap();
...
async fn script_loop_context() {
    let bus = FlowBus::new();
    bus.register_script_typed("it", "rhai", ScriptKind::Iterator, "[10, 20, 30]").unwrap();
    bus.register_script("acc", "rhai", r#"
...
async fn route_chain_matched() {
    let bus = FlowBus::new();
    bus.register("r1", cmp(|ctx| async move {
        let v: i64 = ctx.request_data::<serde_json::Value>()
            .and_then(|v| v.get("level").and_then(|x| x.as_i64()))
            .unwrap_or(0);
        Ok(json!(v >= 5))
    }));
    bus.register("r2", cmp(|ctx| async move {
        let v: i64 = ctx.request_data::<serde_json::Value>()
            .and_then(|v| v.get("level").and_then(|x| x.as_i64()))
            .unwrap_or(0);
        Ok(json!(v < 5))
    ...
    bus.add_route_chain("vipChain", "order", "r1", "THEN(vip)").unwrap();
    bus.add_route_chain("normalChain", "order", "r2", "THEN(normal)").unwrap();

    let resps = bus.execute_route_chain(Some("order"), json!({"level": 8})).await.unwrap();
    assert_eq!(resps.len(), 1);
    assert_eq!(resps[0].chain_id, "vipChain");
    assert_eq!(resps[0].data("plan"), Some(json!("vip")));

    let resps = bus.execute_route_chain(Some("order"), json!({"level": 1})).await.unwrap();
    assert_eq!(resps.len(), 1);
...
async fn route_chain_no_match_error() {
    ...
    let err = bus.execute_route_chain(Some("ns"), Value::Null).await.unwrap_err();
    assert!(matches!(err, LiteflowError::NoMatchedRouteChain));
    // namespace 无 route 链路
    let err = bus.execute_route_chain(Some("other"), Value::Null).await.unwrap_err();
...
async fn route_via_xml_and_json() {
    ...
    rule::load_xml_str(&bus, xml).unwrap();
    let resps = bus.execute_route_chain(Some("ns1"), json!({"flag": true})).await.unwrap();
    assert_eq!(resps.len(), 1);
    ...
    let resps = bus.execute_route_chain(Some("ns2"), json!({"flag": true})).await.unwrap();
    assert_eq!(resps.len(), 1);
