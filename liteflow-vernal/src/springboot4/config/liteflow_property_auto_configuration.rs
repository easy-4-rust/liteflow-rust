use crate::springboot4::{LiteflowMonitorProperty, LiteflowProperty};
use crate::{LiteflowRuleFormat, LiteflowVernalConfig};

/// 合并 Spring Boot 4 主属性与监控属性的自动配置对象。
///
/// Boot 4 使用 `@AutoConfiguration`，但字段合并契约与 Java Boot 3 starter
/// 保持一致。对应 Java:
...
impl LiteflowPropertyAutoConfiguration {
    ...
    ///
    /// # 返回
    /// 可直接交给 Boot 4 主自动配置的统一配置。对应 Java:
    ...
    pub fn liteflow_config(
        ...
        // Java 自动配置逐字段写入 LiteflowConfig；这里不借用 Boot 3 属性对象，
        // 确保 Boot 4 对象自身承担完整绑定和合并逻辑。
        LiteflowVernalConfig {
